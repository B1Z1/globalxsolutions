const contentfulPrototypes = require('./contentful-prototypes')
const contentfulConceptions = require('./contentful-conceptions')
const contentfulSolutions = require('./contentful-solutions')
const contentfulNewsroom = require('./contentful-newsroom')
const contentfulEvents = require('./contentful-events')
const contentfulMediaAboutUs = require('./contentful-media-about-us')

module.exports = {
  contentfulPrototypes,
  contentfulConceptions,
  contentfulSolutions,
  contentfulNewsroom,
  contentfulEvents,
  contentfulMediaAboutUs,
}
